/**
 * Every recharts-backed chart on the Visualization Dashboard.
 *
 * Split out of Visualizations.tsx so that recharts — which is by a wide
 * margin the heaviest dependency in the bundle — lands in its own chunk and
 * is fetched only once the page shell has painted. The page itself is
 * already a lazy route, so moving recharts into a vendor chunk would have
 * made the number in the build output smaller without making the route any
 * faster to open; deferring it *within* the page is what actually shortens
 * time to first paint here.
 */
